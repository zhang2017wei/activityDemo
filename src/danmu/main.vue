<template>
  <div ref="container" class="vue-danmaku">
    <div
      :class="['danmus', { show: !hidden }, { paused: paused }]"
      ref="dmContainer"
    ></div>
    <slot />
  </div>
...
export default {
  ...
  methods: {
    ...
    insert() {
      const index = this.danmaku.loop
        ? this.index % this.danmuList.length
        : this.index
      ...
      this.$nextTick(() => {
        ...
        if (!this.channels) {
          this.calcChannels = Math.floor(
            this.container.height / (this.danmu.height + this.danmu.top)
          )
        }
        ...
        if (channelIndex >= 0) {
          ...
          el.addEventListener('animationend', () => {
            if (
              Number(el.dataset.index) === this.danmuList.length - 1 &&
              !this.danmaku.loop
            ) {
    ...
    getDanRight(el) {
      ...
      const eleRight =
        ...
        this.$dmContainer.getBoundingClientRect().right + eleWidth
